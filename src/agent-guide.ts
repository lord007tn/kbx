...
4. kbx_search performs a bounded freshness refresh when the detected change count is small. If freshness is skipped or clearly stale, call kbx_refresh_file for a known file or kbx_refresh_index for a broader refresh.
5. Start with top_k=5. Use top_k=10 only for broad discovery or ambiguous terms.
6. Treat kbx_search results as previews. Read the id, source, chunk_idx, score, match, preview, and freshness fields.
7. Call kbx_get_chunk for the specific ids you intend to rely on or quote.
8. Search again with exact symbols, config keys, error strings, filenames, or acronyms when the first query is too broad.
9. Prefer citing source and chunk_idx in your response. If source is "external:..." treat it as imported local context, not a repository path.
